  ...
  render() {
        ...
        .map(trx => {
          ...
          return (
            <tr key={"my_history_" + keyIndex}>
              <td style={{ width: "25%", textAlign: "center" }} className={parsed_order.className}>
                <PriceText preFormattedPrice={parsed_order} />
              </td>
              <td style={{ width: "15%", textAlign: "center" }}>
                {!isAsk ? <Translate content="exchange.sell" /> : <Translate content="exchange.buy" />}
              </td>
              <td style={{ width: "20%", textAlign: "center" }}>{parsed_order.receives}</td>
              <td style={{ width: "20%", textAlign: "center" }}>{parsed_order.pays}</td>
              <td style={{ width: "20%", textAlign: "center" }}><Link to={`/block/${block_num}`}>#{utils.format_number(block_num, 0)}</Link></td>
            </tr>
          );
        }).toArray();
        ...
        .map(order => {
          ...
          return (
            <tr key={"history_" + keyIndex}>
              ...
              <td style={{ width: "20%", textAlign: "center" }}>{parsed_order.receives}</td>
              <td style={{ width: "20%", textAlign: "center" }}>{parsed_order.pays}</td>
              <td style={{ width: "20%", textAlign: "center" }} data-tip={new Date(order.time)}>{parsed_order.time}</td>
            </tr>
          );
        }).toArray();
    }
    ...
    return (
      <div className={this.props.className}>
        <div style={this.props.headerStyle} className="grid-block shrink left-orderbook-header">
          ...
          <table className="table order-table text-right market-right-padding">
            <thead>
              <tr>
                ...
                <th style={{ width: "15%", textAlign: "center" }}><Translate className="header-sub-title" content="exchange.buyOrSell" /></th>
                <th style={{ width: "20%", textAlign: "center" }}><span className="header-sub-title"><AssetName dataPlace="top" name={quoteSymbol} /></span></th>
                <th style={{ width: "20%", textAlign: "center" }}><span className="header-sub-title"><AssetName dataPlace="top" name={baseSymbol} /></span></th>
                <th style={{ width: "20%", textAlign: "center" }}><Translate className="header-sub-title" content={activeTab === "history" ? "explorer.block.date" : "explorer.block.title"} /></th>
              </tr>
            </thead>
          </table>
        </div>
        <div
          ...
          ref="history"
        >
          <table className="table order-table text-right market-right-padding table-4">
            <TransitionWrapper
              component="tbody"
