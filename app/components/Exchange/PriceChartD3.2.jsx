...
const getGrid = (gridValue) => {
  return ({
    innerTickSize: -1 * gridValue,
...
class CandleStickChartWithZoomPan extends React.Component {
  constructor(props) {
    ...
    const pricePrecision = props
      .base
      .get("precision");
    const volumePrecision = props
      .quote
      .get("precision");
    ...
    this.onTrendLineComplete = this
      .onTrendLineComplete
      .bind(this);
    this.onFibComplete = this
      .onFibComplete
      .bind(this);
  ...
  componentWillReceiveProps(np) {
    ...
    if (!utils.are_equal_shallow(np.indicators, this.props.indicators) || !utils.are_equal_shallow(np.indicatorSettings, this.props.indicatorSettings)) {
      this.setState({
        calculators: this._getCalculators(np)
      });
    }
  }
  ...
  _getCalculators(props = this.props) {
    ...
    calculators.sma = sma().windowSize(parseInt(indicatorSettings["sma"], 10))
      .sourcePath("close")
      .stroke("#1f77b4")
      .fill("#1f77b4")
      .merge((d, c) => {
    ...
    calculators.ema1 = ema().windowSize(parseInt(indicatorSettings["ema1"], 10)).merge((d, c) => {
      d.ema1 = c;
    }).accessor(d => d.ema1);

    calculators.ema2 = ema().windowSize(parseInt(indicatorSettings["ema2"], 10)).merge((d, c) => {
      d.ema2 = c;
    }).accessor(d => d.ema2);

    calculators.smaVolume = sma().windowSize(parseInt(indicatorSettings["smaVolume"], 10))
      .sourcePath("volume")
      .merge((d, c) => {
        d.smaVolume = c;
      })
    ...
    calculators.bb = bollingerBand().merge((d, c) => {
      d.bb = c;
    }).accessor(d => d.bb);
  ...
  _renderVolumeChart(chartMultiplier) {
    ...
    const { axisLineColor, volumeColor, positiveColor, negativeColor, indicatorLineColor } = this._getThemeColors();
    ...
    return <Chart
      ...
      height={height * 0.2}
      origin={(w, h) => [
        0, h - (chartMultiplier * height * 0.2)
      ]}>
      ...
      <YAxis
        ...
        orient="left"
        ticks={4}
        tickFormat={volumeFormat} /> {indicators.macd
          ? null
          : <MouseCoordinateX
            id={1}
            rectWidth={125}
      ...
      <BarSeries yAccessor={d => d.volume} fill={
        (d) => d.close > d.open ? positiveColor : negativeColor
      } /> {indicators.smaVolume
        ? <AreaSeries
          yAccessor={calculators
            .smaVolume
            .accessor()}
          stroke={calculators
            .smaVolume
      ...
      <EdgeIndicator
        ...
        yAccessor={d => d.volume}
        displayFormat={volumeFormat}
        fill="#0F0F0F" /> {indicators.smaVolume
          ? <EdgeIndicator
            lineStroke={indicatorLineColor}
            rectWidth={65}
            itemType="first"
  ...
  _renderCandleStickChart(chartMultiplier, maCalcs) {
    const { height, width, showVolumeChart, indicators, onMouseMove } = this.props;
    const {
      timeFormatter,
      volumeFormat,
      priceFormat,
    ...
    return <Chart
      id={1}
      height={height * ((!chartMultiplier
        ? 1
        : 0.9) - (0.20 * chartMultiplier))}
      yExtents={[
        d => [
          d.high, d.low
        ],
      ...
      {indicators.macd || showVolumeChart
        ? null
        : <XAxis
          tickStroke={axisLineColor}
          stroke={axisLineColor}
      ...
      <YAxis
        ...
        ticks={5}
        tickStroke={axisLineColor}
        stroke={axisLineColor} /> {indicators.macd || showVolumeChart
          ? null
          : <MouseCoordinateX
            id={1}
            rectWidth={125}
      ...
      <OHLCTooltip
        ...
        accessor={(d) => {
          // 这里使用了一个有副作用的accessor是非常不明智的，但暂时先用着
          let fixedData = {
            date: timeFormatter(d.date),
            open: priceFormat(d.open),
            high: priceFormat(d.high),
            low: priceFormat(d.low),
            close: priceFormat(d.close),
            volume: volumeFormat(d.volume)
          };
          onMouseMove && onMouseMove(fixedData);
          return fixedData;
        }}
        forChart={1}
        ...
        origin={[-40, -10]} /> {maCalcs.length
          ? <MovingAverageTooltip origin={[-40, 0]} calculators={maCalcs} />
          : null}


      {indicators.bb
        ? <BollingerBandTooltip origin={[-40, 40]} calculator={calculators.bb} />
        : null}
  ...
  render() {
    ...
    const yGrid = getGrid(width - margin.left - margin.right);
    const xGrid = getGrid(width - margin.top - margin.bottom);
    const themeColors = colors[theme];
    const { axisLineColor, indicatorLineColor } = themeColors;
    let chartMultiplier = showVolumeChart
      ? 1
      : 0; // Used to adjust the height of the charts and their positioning
    // if (indicators.bb) calc.push(bb); Indicator calculators
    let calc = [],
      maCalcs = [],
      ...
      <ChartCanvas
        ...
        {indicators.macd
          ? <Chart
            id={3}
            height={height * 0.2}
            yExtents={calculators
              .macd
              .accessor()}
            origin={(w, h) => [
              0, h - ((chartMultiplier - (showVolumeChart
                ? 1
                : 0)) * height * 0.2)
            ]}
            padding={{
          ...
          : <span></span>/* Need to return an empty element here, null triggers an error */}
...
export default class Wrapper extends React.Component {
  ...
  render() {
    const { currentPeriod, buckets, bucketSize, indicators, indicatorSettings } = this.props;
    const { dropdowns } = this.state;
      ...
      .map(bucket => {
        return <div
          ...
          onClick={this
            .props
            .changeBucketSize
            .bind(this, bucket)}>{bucketText(bucket)}</div>;
      });
    ...
    let zoomOptions = zoomPeriods.map(period => {
      return <div
        ...
        onClick={this
          .props
          .changeZoomPeriod
          .bind(this, period)}>{bucketText(period)}</div>;
    });

    /* Indicators dropdown */
    const indicatorOptionsVolume = [];
    const indicatorOptionsPrice = Object
      .keys(indicators)
      .map(i => {
        ...
        let settingInput = hasSetting
          ? <div
            ...
            <div
              ...
              style={{
                paddingRight: 5
              }}><Translate content="exchange.chart_options.period" />:</div>
            <input
              style={{
                margin: 0
              }}
            ...
            <li className="indicator" key={i}>
              ...
              <div
                onClick={this
                  ...
                  .onChangeIndicators
                  .bind(null, i)}
                className="clickable"><Translate content={`exchange.chart_options.${i}`} /></div>
              {settingInput}
            </li>
          );
        } else {
            ...
            <li className="indicator" key={i}>
              <input
                ...
                onClick={this
                  ...
                  .onChangeIndicators
                  .bind(null, i)} />
              <div className="clickable"><Translate
    ...
    /* Tools dropdown */
    const toolsOptions = Object
      .keys(this.props.tools)
      .map(i => {
        return (
          <li
            className="clickable"
                ...
                ? <div className="custom-dropdown-content" onClick={this._stopPropagation}>
                  <ul>
                    ...
                    {indicatorOptionsVolume.length
                      ? <li className="indicator-title"><Translate content="exchange.chart_options.volume_title" /></li>
                      : null}
