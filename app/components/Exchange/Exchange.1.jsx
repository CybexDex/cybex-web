...
import { ChainStore, FetchChain } from "cybexjs/es";
...
// Tooltips
// import { tooltip } from "react-stockcharts";
...
class Exchange extends React.Component {
  ...
  _initialState(props) {
    ...
    [
      ...
      indicators[i[0]] = (i[0] in savedIndicators)
        ? savedIndicators[i[0]]
        : i[1];
    });
    ...
    [
      ...
      indicatorSettings[i[0]] = (i[0] in savedIndicatorsSettings)
  ...
  _getLastMarketKey() {
    const chainID = Apis
      .instance()
      .chain_id;
    return `lastMarket${chainID
      ? ("_" + chainID.substr(0, 8))
      : ""}`;
  }
  ...
  componentDidMount() {
    ...
    console.debug("Exchange State: ", this.state);
    if (centerContainer) {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.quoteAsset.get("symbol") !== this.props.quoteAsset.get("symbol") || nextProps.baseAsset.get("symbol") !== this.props.baseAsset.get("symbol")) {
      ...
      return SettingsActions.changeViewSetting({
        [this._getLastMarketKey()]: nextProps
          .quoteAsset
          .get("symbol") + "_" + nextProps
            .baseAsset
            .get("symbol")
      });
    }
  ...
  _getFee(asset) {
    ...
    const coreFee = new Asset({ amount: fee });
    if (!asset || asset.get("id") === "1.3.0")
      return coreFee;
    ...
    const cerBase = new Asset({
      ...
      precision: ChainStore
        .getAsset(cer.base.asset_id)
        .get("precision")
    });
    const cerQuote = new Asset({
      ...
      precision: ChainStore
        .getAsset(cer.quote.asset_id)
        .get("precision")
    });
    try {
      const cerPrice = new Price({ base: cerBase, quote: cerQuote });
  ...
  _createLimitOrderConfirm(buyAsset, sellAsset, sellBalance, coreBalance, feeAsset, type, short = true, e) {
    ...
    let current = this.state[type === "sell"
      ? "ask"
      : "bid"];
    ...
    if (!feeID) {
      return notify.addNotification({ message: "Insufficient funds to pay fees", level: "error" });
    }
    ...
    if (type === "buy" && lowestAsk) {
      let diff = this
        ...
        .bid
        .price
        .toReal() / lowestAsk.getPrice();
      if (diff > 1.20) {
        this
          .refs
          .buy
          .show();
        return this.setState({ buyDiff: diff });
      }
      ...
      if (diff > 1.20) {
        this
          ...
          .sell
          .show();
        return this.setState({ sellDiff: diff });
      }
    }
    ...
    if (current.for_sale.gt(sellBalance) && !isPredictionMarket) {
      return notify.addNotification({
        message: "Insufficient funds to place order, you need at least " + current
          .for_sale
          .getAmount({ real: true }) + " " + sellAsset.get("symbol"),
        level: "error"
      });
    }
    //
    if (!(current.for_sale.getAmount() > 0 && current.to_receive.getAmount() > 0)) {
      return notify.addNotification({ message: "Please enter a valid amount and price", level: "error" });
    }
    //
        ...
        if (result.error) {
          if (result.error.message !== "wallet locked")
            notify.addNotification({
              message: "Unknown error. Failed to place order for " + current
                .to_receive
                .getAmount({ real: true }) + " " + current.to_receive.asset_id,
              level: "error"
            });
        }
  ...
  _createPredictionShort(feeID) {
    ...
    Promise
      .all([FetchChain("getAsset", this.props.quoteAsset.getIn(["bitasset", "options", "short_backing_asset"]))])
      .then(assets => {
  ...
  _cancelLimitOrder(orderID, e) {
    ...
    MarketsActions.cancelLimitOrder(currentAccount.get("id"), orderID // order id to cancel
    );
  }
  ...
  _changeBucketSize(size, e) {
    ...
    if (size !== this.props.bucketSize) {
      ...
      MarketsActions
        .unSubscribeMarket
        .defer(currentSub[0], currentSub[1]);
      this
        .props
        .subToMarket(this.props, size);
    }
  }
  ...
  _changeZoomPeriod(size, e) {
    ...
    if (size !== this.state.currentPeriod) {
      this.setState({ currentPeriod: size });
      SettingsActions.changeViewSetting({ currentPeriod: size });
    }
  }
  ...
  _currentPriceClick(type, price) {
    ...
    current.price = price[(isBid)
      ? "invert"
      : "clone"]();
    current.priceText = current
      .price
      .toReal();
    if (isBid) {
  ...
  _changeIndicator(key) {
    ...
    SettingsActions.changeViewSetting({ indicators });
  }
  ...
  _changeIndicatorSetting(key, e) {
    ...
    this.setState({ indicatorSettings: indicatorSettings });

    SettingsActions.changeViewSetting({ indicatorSettings: indicatorSettings });
  }

  onChangeFeeAsset(type, e) {
    e.preventDefault();
    if (type === "buy") {
      this.setState({ buyFeeAssetIdx: e.target.value });

      SettingsActions.changeViewSetting({ "buyFeeAssetIdx": e.target.value });
    } else {
      this.setState({ sellFeeAssetIdx: e.target.value });

      SettingsActions.changeViewSetting({ "sellFeeAssetIdx": e.target.value });
    }
  }
  ...
  onChangeChartHeight({ value, increase }) {
    let newHeight = value
      ? value
      : this.state.chartHeight + (increase
        ? 20
        : -20);
    console.log("newHeight", newHeight);
    this.setState({ chartHeight: newHeight });

    SettingsActions.changeViewSetting({ "chartHeight": newHeight });
  }
  ...
  _getFeeAssets(quote, base, coreAsset) {
    ...
    sellFeeAssets = sellFeeAssets.filter(a => {
      ...
      return balances[a.get("id")].balance > balances[a.get("id")]
        .fee
        .getAmount();
    });
  ...
  _setReceive(state, isBid) {
    if (state.price.isValid() && state.for_sale.hasAmount()) {
      state.to_receive = state
        .for_sale
        .times(state.price, isBid);
      state.toReceiveText = state
        .to_receive
        .getAmount({ real: true })
        .toString();
      return true;
  ...
  _setForSale(state, isBid) {
    if (state.price.isValid() && state.to_receive.hasAmount()) {
      state.for_sale = state
        .to_receive
        .times(state.price, isBid);
      state.forSaleText = state
        .for_sale
        .getAmount({ real: true })
        .toString();
      return true;
  ...
  render() {
    ...
    const showVolumeChart = this
      .props
      .viewSettings
      .get("showVolumeChart", true);
    ...
    if (activeMarketHistory.size) {
      ...
      if (second_latest) {
        ...
        changeClass = latestPrice.full === oldPrice.full
          ? ""
    ...
    let orderMultiplier = leftOrderBook
      ? 2
      : 1;
    ...
    let buyForm = isFrozen
      ? null
      : (<BuySell
        onBorrow={!isNullAccount && baseIsBitAsset
          ? this
            ._borrowBase
            .bind(this)
          : null}
        ...
        backedCoin={this
          .props
          .backedCoins
          .find(a => a.symbol === base.get("symbol"))}
        currentBridges={this
          .props
          .bridgeCoins
          .get(base.get("symbol")) || null}
        smallScreen={smallScreen}
        isOpen={this.state.buySellOpen}
        onToggleOpen={this
          ._toggleOpenBuySell
          .bind(this)}
        className={cnames("small-12 no-padding middle-content", {
          disabled: isNullAccount
        }, leftOrderBook || smallScreen
            ? "medium-6"
            : "medium-6 xlarge-4", this.state.flipBuySell
            ? `order-${buySellTop
              ? 2
        ...
        onSubmit={this
          ...
          .bind(this, quote, base, baseBalance, coreBalance, buyFeeAsset, "buy")}
        ...
        onChangeFeeAsset={this
          .onChangeFeeAsset
          .bind(this, "buy")}
        isPredictionMarket={base.getIn(["bitasset", "is_prediction_market"])}
        onFlip={!this.state.flipBuySell
          ? this
            ._flipBuySell
            .bind(this)
          : null}
        onTogglePosition={this
          ._toggleBuySellPosition
          .bind(this)} />);

    let sellForm = isFrozen
      ? null
      : (<BuySell
        onBorrow={!isNullAccount && quoteIsBitAsset
          ? this
            ._borrowQuote
            .bind(this)
          : null}
        ...
        backedCoin={this
          .props
          .backedCoins
          .find(a => a.symbol === quote.get("symbol"))}
        currentBridges={this
          .props
          .bridgeCoins
          .get(quote.get("symbol")) || null}
        smallScreen={smallScreen}
        isOpen={this.state.buySellOpen}
        onToggleOpen={this
          ._toggleOpenBuySell
        ...
        onSubmit={this
          ._createLimitOrderConfirm
          .bind(this, base, quote, quoteBalance, coreBalance, sellFeeAsset, "sell")}
    ...
    let orderBook = (<OrderBook
      ...
      wrapperClass={`order-${buySellTop
        ? 3
        : 1} xlarge-order-${buySellTop
          ? 4
          : 1}`} />);

    // let chartHeight = this.state.height > 1100 ? this.state.chartHeight :
    // this.state.chartHeight - 125;
    let fixedChartHeight = this.state.height - 185;
    return (
      <div className="grid-block page-layout market-layout">
        <AccountNotifications /> {/* Main vertical block with content */}
        ...
        <div className={cnames("grid-block main-content vertical no-overflow")}>
          ...
          <ExchangeHeader
            ...
            currentMouseItem={currentMouseItem}
            onSelectIndicators={this
              ._onSelectIndicators
              .bind(this)}
          ...
          <div className="grid-block vertical no-padding bgcolor-primary" id="CenterContent" ref="center">
            {!showDepthChart
              ? (
                <div className="grid-block shrink no-overflow" id="market-charts">
                  ...
                  <PriceChartD3
                    ...
                    changeBucketSize={this
                      ._changeBucketSize
                      .bind(this)}
                    changeZoomPeriod={this
                      ._changeZoomPeriod
                      .bind(this)}
                    onSelectIndicators={this
                      ._onSelectIndicators
                      .bind(this)}
                    onChangeIndicators={this
                      ._changeIndicator
                      .bind(this)}
                    onChangeTool={(key) => {
                    ...
                    onChangeIndicatorSetting={this
                      ._changeIndicatorSetting
                      .bind(this)}
                    onMouseMove={
                      this
                        ._handleMouseMove
              ...
              : (
                <div className="grid-block vertical no-padding shrink">
                  <DepthHighChart
                    ...
                    flat_asks={flatAsks}
                    flat_bids={flatBids}
                    flat_calls={showCallLimit
                      ? flatCalls
                      : []}
                    flat_settles={this
                      .props
                      .settings
                      .get("showSettles") && flatSettles}
