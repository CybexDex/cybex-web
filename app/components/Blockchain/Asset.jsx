...
class Asset extends React.Component {
  ...
  renderSummary(asset) {
    ...
    let vestingPeriod =
      options && options.extensions
        ? getExtension(options.extensions, "vesting_period")
        : null;
    vestingPeriod = vestingPeriod ? (
      <tr>
        <td>
          {" "}
          <Translate content="explorer.asset.summary.vesting" />{" "}
        </td>
        <td>{vestingPeriod}</td>
      </tr>
    ) : null;
  ...
  renderPriceFeedData(asset) {
    ...
    let sortFunctions = {
      ...
      collateral: function(a, b) {
        return (
          (sortDirection ? 1 : -1) *
          (b.getCollateral().getAmount() - a.getCollateral().getAmount())
        );
      },
      debt: function(a, b) {
        return (
          (sortDirection ? 1 : -1) *
          (b.amountToReceive().getAmount() - a.amountToReceive().getAmount())
        );
      },
      ratio: function(a, b) {
          ...
          <div className="asset-card">
            ...
            >
              <Tab title="explorer.asset.price_feed_data.title">
                <table
                  className="table asset-table order-table table-hover"
                  style={{ padding: "1.2rem" }}
                >
                  {header}
                  <tbody>{rows}</tbody>
                      ...
                      <th style={{ paddingRight: 10 }} className="clickable">
                        ...
                        {this.state.callOrders.length ? (
                          <span>
                            &nbsp;(<FormattedPrice
                              base_amount={
                                this.state.callOrders[0].call_price.base.amount
                              }
                              base_asset={
                              ...
                              quote_amount={
                                this.state.callOrders[0].call_price.quote.amount
                              }
                              quote_asset={
                                this.state.callOrders[0].call_price.quote
                                  .asset_id
                              }
                              hide_value
                              noPopOver
  ...
  render() {
    ...
    var priceFeedData =
      "bitasset" in asset ? this.renderPriceFeedData(asset) : null;
    ...
    return (
      <div className="grid-block page-layout">
        <div
          className="grid-block main-content vertical"
          style={{ overflow: "visible" }}
        >
          <div className="grid-container">
            <div className="grid-content">
              <div
                className="grid-block no-margin small-12 shrink"
                style={{ overflow: "visible" }}
              >
                {this.renderAboutBox(asset)}
              </div>
              <div
                className="grid-block no-margin small-12 shrink vertical medium-horizontal"
                style={{ overflow: "visible" }}
              >
                <div
