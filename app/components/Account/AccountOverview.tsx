...
let AccountOverview = class extends React.Component<any, any> {
  ...
  _renderBalances(balanceList, optionalAssets, visible?) {
    ...
    balanceList.forEach(balance => {
      ...
      const hasBalance = !!balanceObject && !!balanceObject.get("balance");
      ...
      const canWithdraw =
        ...
        (hasBalance && !!balanceObject && balanceObject.get("balance") != 0);
        ...
        <tr key={asset.get("symbol")} style={{ maxWidth: "100rem" }}>
            ...
            <EquivalentPrice
              refCallback={c => {
                if (c && c.refs.bound_component)
                  this.priceRefs[asset.get("symbol")] = c.refs.bound_component;
              }}
          ...
          <td style={{ textAlign: "right" }} className="column-hide-small">
            <Market24HourChangeComponent
              refCallback={c => {
                if (c && c.refs.bound_component)
                  this.changeRefs[asset.get("symbol")] = c.refs.bound_component;
              }}
              fromAsset={asset.get("id")}
          ...
          <td>
            {isBitAsset ? (
              <div
                ...
                data-place="bottom"
                data-tip={counterpart.translate("tooltip.borrow", {
                  asset: symbol
                })}
              >
                {borrowLink}
          ...
          <td>
            {isBitAsset ? (
              <div
                className="inline-block"
                data-place="bottom"
                data-tip={counterpart.translate("tooltip.settle", {
                  asset: symbol
                })}
              >
                {settleLink}
  ...
  render() {
    ...
    let portFolioValue = (
      <TotalBalanceValue noTip balances={totalBalanceList} hide_asset />
    );
    let ordersValue = (
      <TotalBalanceValue
        noTip
                ...
                <div className="hide-selector">
                  <div
                    className={cnames("hide-selector-item", {
                      inactive: showHidden && hiddenBalances.length
                    })}
                    onClick={
                      showHidden
                        ? this._toggleHiddenAssets.bind(this)
                        : () => {}
                    }
                  >
