...
class AccountAssets extends React.Component {
  ...
  _issueButtonClick(asset_id, symbol, asset, e) {
    e.preventDefault();
    let { issue } = this.state;
      ...
      .map(asset => {
        ...
        return (
          <tr key={asset.symbol}>
            ...
            <td>
              <FormattedAsset
                amount={parseInt(asset.dynamic.current_supply, 10)}
                asset={asset.id}
              />
            </td>
            <td>
            ...
            <td>
              {assedIsInited ? (
                <Translate
                  component="button"
                  className="button disabled outline"
                  content="transaction.trxTypes.crowd_inited"
                />
              ) : (
                <button
                  onClick={() => this._showInitCrowdModal(asset)}
                  className="button outline"
                >
        ...
        <BaseModal id="initCrowdModal" overlay={true}>
          ...
          <div className="grid-block vertical">
            <CrowdInitModal
              ...
              onClose={() => {
                ZfApi.publish("initCrowdModal", "close");
              }}
              asset={assetToIssue}
            />
          </div>
