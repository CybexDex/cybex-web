...
class AccountDepositWithdraw extends React.Component {
  ...
  renderServices(openLedgerGatewayCoins, rudexGatewayCoins) {
      ...
      template: (
        <div className="content-block">
          ...
          {rudexService === "gateway" && rudexGatewayCoins.length ? (
            <RuDexGateway account={account} coins={rudexGatewayCoins} />
          ) : null}
    ...
    serList.push({
      name: "BitKapital",
      template: (
        <BitKapital viewSettings={this.props.viewSettings} account={account} />
      )
    });
...
class DepositStoreWrapper extends React.Component {
  componentWillMount() {
    if (Apis.instance().chain_id.substr(0, 8) === "4018d784") {
      // Only fetch this when on CYB main net
