...
class MarginPosition extends React.Component {
  ...
  render() {
    ...
    const marketURL = `/market/${debtAsset.get("symbol")}_${collateralAsset.get(
      "symbol"
    )}`;
    ...
    objects.forEach(object => {
      if (object) {
        let index = optionals.findIndex(o => {
          return (
            o &&
            o.get("id") === object.getIn(["call_price", "quote", "asset_id"])
          );
        });
        if (index !== -1) {
    ...
    <table className={"table " + className}>
        ...
        <tr>
          ...
          <th>
            <div
              ...
              data-place="top"
              data-tip={counterpart.translate("tooltip.coll_ratio")}
            >
          ...
          <th style={alignRight} className="column-hide-small">
            <div
              className="tooltip inline-block"
              data-place="top"
              data-tip={counterpart.translate("tooltip.call_price")}
            >
      ...
      <tbody>
        {callOrders
          ...
          .map(id => (
            <MarginPositionWrapper key={id} object={id} account={account} />
          ))}
      </tbody>
      <PlaceHolderWrapper
        account={account}
