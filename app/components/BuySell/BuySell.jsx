...
class UserOrder {
  ...
  setTotal = e => {
    ...
    this.total = utils.get_asset_amount(this.totalSats, {
      precision: this.tp.int
    });
    if (this.price) {
      this._deriveAmount();
    } else {
  ...
  _deriveAmount() {
    ...
    this.amount = utils.get_asset_amount(this.amountSats, {
      precision: this.ap.int
    });
    console.log(
      "this.totalSats:",
      this.totalSats,
    ...
    let satPrice = utils.get_asset_amount(this.totalSats / this.amountSats, {
      precision: this.tp.int - this.ap.int
    });
...
class BuySell extends React.Component {
  ...
  componentWillReceiveProps(np) {
    if (
      np.baseAsset &&
      np.baseAsset.getIn(["bitasset", "is_prediction_market"])
    ) {
      // console.log("this.props:", this.props);
      this.props.history.push(
        `market/${np.baseAsset.get("symbol")}_${np.quoteAsset.get("symbol")}`
      );
    }
    ...
    if (
      np.quoteAsset.get("symbol") !== this.props.quoteAsset.get("symbol") ||
      np.baseAsset.get("symbol") !== this.props.baseAsset.get("symbol")
    ) {
      ...
      SettingsActions.changeViewSetting({
        [this._getLastMarketKey()]:
          np.quoteAsset.get("symbol") + "_" + np.baseAsset.get("symbol")
      });
  ...
  _renderRows(bids, isBid) {
    let rows = bids
      .sort((a, b) => {
        return isBid
          ? b.price_full - a.price_full
          : a.price_full - b.price_full;
      })
      .slice(0, 7)
      .map((a, index) => {
        return (
  ...
  _getBalance(b, asset, isBid) {
    ...
    return (
      <span>
        <span onClick={onClick} className="BuySell__balance">
  ...
  render() {
    ...
    return (
      <div className="grid-block page-layout">
        ...
        >
          <div
            className="grid-content large-offset-2 shrink"
              ...
              <div className="grid-block BuySell__inputs">
                <div className="grid-content small-6">
                  <div className="text-center inputs__header">
                    <h4>
                      Buy <AssetName name={quoteSymbol} />
                    </h4>
                    <div>
                      Available:{" "}
                      {this._getBalance(baseBalance, baseAsset, true)}
                    </div>
                  </div>
                  <div className="inputs__input">
                    <div>Amount to Buy:</div>
                    <input
                      value={buyOrder.getAmount()}
                      onChange={buyOrder.setAmount}
                      type="text"
                    />
                  ...
                  <div className="inputs__input">
                    <div>Buy Price:</div>
                    <input
                      value={buyOrder.getPrice()}
                      onChange={buyOrder.setPrice}
                      type="text"
                    />
                ...
                <div className="grid-content small-6">
                  <div className="text-center inputs__header">
                    <h4>
                      Sell <AssetName name={quoteSymbol} />
                    </h4>
                    <div>
                      Available:{" "}
                      {this._getBalance(quoteBalance, quoteAsset, false)}
                    </div>
                  </div>
                  ...
                  <div className="inputs__input">
                    ...
                    <input
                      value={sellOrder.getAmount()}
                      onChange={sellOrder.setAmount}
                      type="text"
                    />
                    <AssetName className="inputs__symbol" name={quoteSymbol} />
                  </div>
                  ...
                  <div className="inputs__input">
                    <div>Sell Price:</div>
                    <input onChange={sellOrder.setPrice} type="text" />
                    <AssetName className="inputs__symbol" name={baseSymbol} />
                  </div>
                    ...
                    <thead>
                      <tr>
                        <th style={{ textAlign: "right" }}>
                          Price ({baseAsset.get("symbol")})
                        </th>
                        <th style={{ textAlign: "right" }}>
                          Amount ({quoteAsset.get("symbol")})
                        </th>
                        <th style={{ textAlign: "right" }}>
                          Value ({baseAsset.get("symbol")})
                        </th>
                      </tr>
                    </thead>
                    <tbody>{this._renderRows(this.props.bids, true)}</tbody>
                  </table>
                </div>
                <div className="grid-content small-6">
                  <table className="table">
                    ...
                    <tbody>{this._renderRows(this.props.asks, false)}</tbody>
                  </table>
                </div>
              </div>
            </div>
