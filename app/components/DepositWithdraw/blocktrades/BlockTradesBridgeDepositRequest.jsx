  ...
  render() {
    let conversion_button = (
      <ButtonConversion
        ...
        url={this.props.url}
        balance={
          this.props.account.get("balances").toJS()[this.props.asset.get("id")]
        }
      />
    );
...
class ButtonWithdraw extends React.Component {
  ...
  render() {
    ...
    return (
      <span>
        <span>
          <button className={button_class} onClick={this.onWithdraw.bind(this)}>
            <Translate content="" />
...
class BlockTradesBridgeDepositRequest extends React.Component {
    ...
    Promise.all([
      ...
      .then(json_responses => {
        ...
        trading_pairs.forEach(pair => {
          ...
          if (
            ...
            if (
              ...
              if (
                input_coin_info.walletType != "Cybex2" &&
                output_coin_info.walletType == "Cybex2"
              ) {
                allowed_mappings_for_deposit[pair.inputCoinType] =
                  allowed_mappings_for_deposit[pair.inputCoinType] || [];
                allowed_mappings_for_deposit[pair.inputCoinType].push(
                  pair.outputCoinType
                );
              } else if (
                input_coin_info.walletType == "Cybex2" &&
                ...
                allowed_mappings_for_withdraw[pair.inputCoinType] =
                  allowed_mappings_for_withdraw[pair.inputCoinType] || [];
                allowed_mappings_for_withdraw[pair.inputCoinType].push(
                  pair.outputCoinType
                );
              } else if (
                input_coin_info.walletType == "Cybex2" &&
                ...
                allowed_mappings_for_conversion[pair.inputCoinType] =
                  allowed_mappings_for_conversion[pair.inputCoinType] || [];
                allowed_mappings_for_conversion[pair.inputCoinType].push(
                  pair.outputCoinType
                );
              }
            }
        ...
        if (allowed_deposit_coin_types.length) {
          ...
          else deposit_input_coin_type = allowed_deposit_coin_types[0];
          ...
          else deposit_output_coin_type = output_coin_types_for_this_input[0];
        }
        ...
        if (allowed_withdraw_coin_types.length) {
          ...
          else withdraw_input_coin_type = allowed_withdraw_coin_types[0];
          let output_coin_types_for_this_input =
          ...
          else withdraw_output_coin_type = output_coin_types_for_this_input[0];
        }
        ...
        if (allowed_conversion_coin_types.length) {
          ...
          else conversion_input_coin_type = allowed_conversion_coin_types[0];
          let output_coin_types_for_this_input =
            allowed_mappings_for_conversion[conversion_input_coin_type];
          if (
  ...
  getCachedOrGeneratedInputAddress(input_coin_type, output_coin_type) {
    ...
    fetch(this.state.url + "/simple-api/initiate-trade", {
      ...
      error => {
        delete this.state.input_address_requests_in_progress[input_coin_type][
          output_coin_type
        ];
        if (
          this.state.deposit_input_coin_type == input_coin_type &&
  ...
  getCachedOrFreshDepositLimit(
    ...
    deposit_limit_promise.then(
      reply => {
        ...
        delete this.state.deposit_limit_requests_in_progress[input_coin_type][
          output_coin_type
        ];
        if (
  ...
  getAndUpdateOutputEstimate(
    ...
    let estimate_output_url =
      this.state.url +
      "timate-output-amount?inputAmount=" +
      ...
      reply => {
        ...
        if (reply.error) {
          if (
            ...
            this.state[deposit_withdraw_or_convert + "_estimate_direction"] ==
              this.estimation_directions.output_from_input
          ) {
  ...
  getAndUpdateInputEstimate(
    ...
    let estimate_input_url =
      this.state.url +
      "timate-input-amount?outputAmount=" +
  ...
  onInputCoinTypeChanged(deposit_withdraw_or_convert, event) {
    ...
    if (!this.state[deposit_withdraw_or_convert + "_estimated_input_amount"]) {
      estimated_input_output_amount = this.getAndUpdateInputEstimate(
        ...
        new_input_coin_type,
        new_output_coin_type,
        this.state[deposit_withdraw_or_convert + "_estimated_output_amount"]
      );
  ...
  render() {
    ...
    if (
      ...
      if (
        Object.getOwnPropertyNames(this.state.allowed_mappings_for_deposit)
          .length > 0
      ) {
        // deposit
        ...
        Object.keys(this.state.allowed_mappings_for_deposit)
          ...
          .forEach(allowed_deposit_input_coin_type => {
            deposit_input_coin_type_options.push(
              ...
              >
                {
                  this.state.coins_by_type[allowed_deposit_input_coin_type]
                    .symbol
                }
              </option>
            );
        ...
        let deposit_output_coin_types = this.state.allowed_mappings_for_deposit[
          this.state.deposit_input_coin_type
        ];
        deposit_output_coin_types.forEach(allowed_deposit_output_coin_type => {
          deposit_output_coin_type_options.push(
            ...
            >
              {
                this.state.coins_by_type[allowed_deposit_output_coin_type]
                  .walletSymbol
              }
            </option>
          );
      ...
      if (
        ...
        Object.keys(this.state.allowed_mappings_for_withdraw)
          ...
          .forEach(allowed_withdraw_input_coin_type => {
            withdraw_input_coin_type_options.push(
              ...
              >
                {
                  this.state.coins_by_type[allowed_withdraw_input_coin_type]
                    .walletSymbol
                }
              </option>
            );
        ...
        withdraw_output_coin_types.forEach(
          allowed_withdraw_output_coin_type => {
            withdraw_output_coin_type_options.push(
              ...
              >
                {
                  this.state.coins_by_type[allowed_withdraw_output_coin_type]
                    .symbol
                }
              </option>
            );
        ...
        let withdraw_button = (
          <ButtonWithdrawContainer
            ...
            asset={
              this.state.coins_by_type[this.state.withdraw_input_coin_type]
                .walletSymbol
            }
            output_coin_name={
              this.state.coins_by_type[this.state.withdraw_output_coin_type]
                .name
            }
            output_coin_symbol={
              this.state.coins_by_type[this.state.withdraw_output_coin_type]
      ...
      if (
        Object.getOwnPropertyNames(this.state.allowed_mappings_for_conversion)
          .length > 0
      ) {
        // conversion
        ...
        Object.keys(this.state.allowed_mappings_for_conversion)
          ...
          .forEach(allowed_conversion_input_coin_type => {
            conversion_input_coin_type_options.push(
              ...
              >
                {
                  this.state.coins_by_type[allowed_conversion_input_coin_type]
                    .walletSymbol
                }
              </option>
            );
        ...
        conversion_output_coin_types.forEach(
          allowed_conversion_output_coin_type => {
            conversion_output_coin_type_options.push(
              ...
              >
                {
                  this.state.coins_by_type[allowed_conversion_output_coin_type]
                    .symbol
                }
              </option>
            );
        ...
        let conversion_button = (
          <ButtonConversionContainer
            asset={
              this.state.coins_by_type[this.state.conversion_input_coin_type]
                .walletSymbol
            }
            account={this.props.account}
