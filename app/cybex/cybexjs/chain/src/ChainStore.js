...
let asset_dynamic_data_prefix =
  "2." + parseInt(impl_object_type.asset_dynamic_data, 10) + ".";
...
class ChainStore {
  ...
  getAsset(id_or_symbol) {
    if (!id_or_symbol || typeof id_or_symbol !== "string") return null;
    // let id_or_symbol = _id_or_symbol.asset_id || _id_or_symbol;
    ...
    if (ChainValidation.is_object_id(id_or_symbol)) {
      ...
      if (
        asset &&
        (asset.get("bitasset") && !asset.getIn(["bitasset", "current_feed"]))
      ) {
        return undefined;
      }
      return asset;
    ...
    if (result === undefined) {
      ...
      Apis.instance()
        ...
        .then(optional_objects => {
          ...
          for (let i = 0; i < optional_objects.length; i++) {
            let optional_object = optional_objects[i];
            if (optional_object) this._updateObject(optional_object, true);
            else {
  ...
  _updateObject(object, notify_subscribers = false, emit = true) {
    ...
    if (
      ...
      object.id.substring(0, call_order_prefix.length) == call_order_prefix
    ) {
      // call_order object
    ...
    if (object.id.substring(0, balance_prefix.length) == balance_prefix) {
      ...
      object.id.substring(0, asset_dynamic_data_prefix.length) ==
      asset_dynamic_data_prefix
    ) {
        ...
        .then(
          vote_obj_array => {
            ...
            for (let i = 0; i < vote_obj_array.length; ++i) {
              if (vote_obj_array[i]) {
                let isWitness =
                  vote_obj_array[i].id.substring(0, witness_prefix.length) ==
                  witness_prefix;
                this._subTo(
