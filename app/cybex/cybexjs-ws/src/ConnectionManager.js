...
class Manager {
  ...
  logFailure(url, err) {
    console.error(
      "Skipping to next full node API server. Error: " +
  ...
  connect(connect = true, url = this.url, enableCrypto = false) {
    return new Promise((resolve, reject) => {
      Apis.instance(url, connect, undefined, enableCrypto)
  ...
  checkConnections(
    ...
    const checkFunction = (resolve, reject) => {
      ...
      let promises = connectionPromises.map(a => a());

      let doPromise = race
        ? () => Promise.race(promises).then(res => [res])
      ...
      doPromise()
        ...
        .catch(e => {
          console.error("CheckConnectionError: ", e);
