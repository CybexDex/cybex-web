...
import { ChainStore, ChainValidation, FetchChain } from "cybexjs";
...
class AccountStore extends BaseStore {
    ...
    this.state.linkedAccounts = this.state.linkedAccounts.withMutations(
      linkedAccounts => {
        account_refs.forEach(id => {
          ...
          if (
            account &&
            !this.state.unFollowedAccounts.includes(account.get("name"))
          ) {
  ...
  getMyAccountsWithAuthState() {
    ...
    return accounts.sort(
      (prev, next) =>
        prev.auth < next.auto ? 1 : prev.auth == next.auto ? 0 : -1
    );
  }
