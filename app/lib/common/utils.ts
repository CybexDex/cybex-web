...
var Utils = {
  ...
  format_price: function(
    ...
    if (inverted) {
      if (
        parseInt(quoteAsset.id.split(".")[2], 10) <
        parseInt(baseAsset.id.split(".")[2], 10)
      ) {
        return `${this.format_number(
          quoteAmount / precision / (baseAmount / basePrecision),
          Math.max(5, quoteAsset.precision),
          trailing_zeros
        )}${!noSymbol ? "" + quoteAsset.symbol + "/" + baseAsset.symbol : ""}`;
      } else {
        return `${this.format_number(
          baseAmount / basePrecision / (quoteAmount / precision),
          Math.max(5, baseAsset.precision),
          trailing_zeros
        )}${!noSymbol ? "" + baseAsset.symbol + "/" + quoteAsset.symbol : ""}`;
      }
    } else {
      if (
        parseInt(quoteAsset.id.split(".")[2], 10) >
  ...
  getFee: function({
    ...
    if (balances && balances.length) {
      balances.forEach(b => {
        if (
          b.get("asset_type") === "1.3.0" &&
          b.get("balance") < coreFee.amount
        ) {
      ...
      balances.forEach(b => {
        if (
          b.get("asset_type") === fee.asset &&
          b.get("balance") < fee.amount
        ) {
  ...
  replaceName(_name, isBitAsset = false) {
    ...
    let prefix = isBitAsset
      ? "cyb."
      : toReplace[i] && !toHidden[toReplace[i]]
