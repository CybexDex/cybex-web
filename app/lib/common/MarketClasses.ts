...
class LimitOrder {
  ...
  constructor(order, assets, market_base) {
    ...
    this.init_for_sale = parseInt(order.sell_price.base.amount, 10);
    let base = new Asset({
      asset_id: order.sell_price.base.asset_id,
      amount: this.init_for_sale,
...
class CallOrder {
  ...
  getPrice(squeeze = true, p = this.call_price, d = 8) {
    if (squeeze) {
  ...
  /*
    * Assume a USD:CYB market
    * The call order will always be selling CYB in order to buy USD
