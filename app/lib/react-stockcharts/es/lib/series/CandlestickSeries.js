...
function roundRect(ctx, x, y, width, height, _radius) {
	var radius = !Array.isArray(_radius) ? new Array(8).fill(_radius * 0.01 * Math.min(width, height)) : _radius.map(function (r) {
		return r * 0.01 * Math.min(width, height);
	});
	...
	ctx.quadraticCurveTo(x + width, y + height, x + width - radius[5], y + height);
	ctx.lineTo(x + radius[6], y + height);
	ctx.quadraticCurveTo(x, y + height, x, y + height - radius[7]);
	ctx.lineTo(x, y + radius[0]);
	ctx.quadraticCurveTo(x, y, x + radius[1], y);
	ctx.closePath();
}
...
function _drawOnCanvas(ctx, props, moreProps) {
   ...
   ctx.stroke(); */
			var d = each.wick;
			if (rectRadius) {
				fillRoundRect(ctx, d.x - 0.5, d.y1, 1, d.y2 - d.y1, rectRadius);
				fillRoundRect(ctx, d.x - 0.5, d.y3, 1, d.y4 - d.y3, rectRadius);
			} else {
     ...
     */
					...
					if (rectRadius) {
						fillRoundRect(ctx, d.x - 0.5, d.y, 1, d.height, rectRadius);
					} else {
     ...
     */
					if (rectRadius) {
						fillRoundRect(ctx, d.x, d.y - 0.5, d.width, 1, rectRadius);
					} else {
     ...
     */
					if (rectRadius) {
						fillRoundRect(ctx, d.x, d.y, d.width, d.height, rectRadius);
					} else {
						ctx.fillRect(d.x, d.y, d.width, d.height);
					}
					if (strokeKey !== "none" && !rectRadius) ctx.strokeRect(d.x, d.y, d.width, d.height);
				}
			});
		});
	});
